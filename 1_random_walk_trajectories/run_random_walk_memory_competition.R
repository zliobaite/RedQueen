...
out_file <- 'generated_data/data_random_memory_competition.csv'
...
for (cik in 1:n_loops){
  ...
  dir_before <- rep(0,(m_taxa_origin*n_times*3))
  ...
  for (time_now in 2:n_times){
    cp_correction <- mean(data_abundance[,(time_now-1)])
